...
void draw_health_bar2(int x, int y, int width, int height, float current_hp, float max_hp, 
...
void game_scene2_draw(Scene* self)
{
    ...
    for (int i = 0; i < allEle.len; i++)
    {
        ...
        if (ele->label == Character_L) {
            ...
            draw_health_bar2(15, 15, 250, 25, character->hp, character->max_hp, 
        ...
        // 绘制敌人血条
            ...
            if (ele->label == BossEnemy_L) {
                ...
                if (boss->hp < max_hp) {
                    draw_health_bar2(bar_x, bar_y, bar_width, bar_height, 
